...
class CRF(object):
    ...
    def get_word_features(self, sent, i):
        ...
        word_feats.update({'fl': sent.tokens[i][0]})
        word_feats.update({'ll': sent.tokens[i][-1]})
        word_feats.update({'f2l': sent.tokens[i][0:2]})
        word_feats.update({'l2l': sent.tokens[i][-3:-1]})
        word_feats.update({'f3l': sent.tokens[i][0:3]})
        word_feats.update({'l3l': sent.tokens[i][-4:-1]})
        if not(i == 0):
           word_feats.update({'prev': sent.tokens[i-1]})
        if not(i == len(sent.tokens) - 1):
            word_feats.update({'next': sent.tokens[i+1]})
        return word_feats
